    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        for (@NotNull Player recipient: server.getOnlinePlayers()) {
            if (recipient.getName().equalsIgnoreCase(args[0])) {
                ...
                player.sendMessage(MiniMessage.miniMessage().deserialize(configFile.getString("outgoing").replace("<recipient>", player.getName()).replace("<message>", stringBuilder)));
                return true;
            }
            player.sendMessage(MiniMessage.miniMessage().deserialize("<red>(!) This user was not found. Check your spelling and try again!"));
            return true;
        }
        return false;
    }
